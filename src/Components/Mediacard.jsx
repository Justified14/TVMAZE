...
export default function Mediacard({id, image, genres, name}) {
  return (
    <Link to={`/tvshows/${id}`}>
        <div className='me-2 p-2 MediaCard position-relative border-bottom-0 rounded-bottom' style={{width:"80vw",height: '350px'}}>
            <img className='w-100 h-100' src={image?.original} alt={name} loading='lazy'/>
            <div className='position-absolute bottom-0 w-100 bg-dark opacity-50 z-0 border-bottom-0 rounded-bottom' style={{height:"96px"}}/>
